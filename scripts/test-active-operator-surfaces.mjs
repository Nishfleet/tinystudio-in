...
try {
	...
	// byte-identically from the hermetic empty root first, but the tracked
	// 11/10 proof-run brief also refuses to regenerate from a state-less root,
	// so give the surface gate one inert outbound pipeline record (no score,
	// touches, or loom) first; it is removed again afterwards.
	...
	// Nested gate and metrics chains inside the loop only consumed the metrics
	// JSON and refreshed a private runs/live-metrics.md, never the tracked
	// surface. Reproduce the tracked live metrics pipeline exactly: 38 new, 5
	// scored (active), and 7 paused (scored) prospect folders = 50 total with
	// 12 scored including inactive, plus 3 unregistered client folders, so the
