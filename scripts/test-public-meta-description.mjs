...
// The site's SEO audit band: meta descriptions are flagged below 70 chars
// (too short to communicate the page job) and above 160 chars (truncation
// risk). The wording is source-bound to the page's own copy: the page is the
// app-specific privacy destination for Promptly, is public ahead of release,
// and describes how professional, client, and support data may be handled.
...
ok(description === EXPECTED_DESCRIPTION, "name=description matches the approved wording")
ok(description.includes("Promptly by Tiny Studio"), "description names the app truthfully")
ok(description.includes("privacy policy") && description.includes("app-specific"), "description states the page is the app-specific privacy policy")
ok(description.includes("ahead of release"), "description states the page is public ahead of release (matches page copy)")
ok(!/[—-]+\s*(pricing|sales|download|subscribe)/i.test(description), "description makes no pricing or sales claim")
