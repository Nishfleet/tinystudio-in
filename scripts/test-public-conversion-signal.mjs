...
const DEFAULT_SUBJECT = "The Website Correction application"

// Candidate-4 regression guard: the offer must never be named with the
// generic "reviewed service" phrasing; repo truth is "The Website Correction".
const GENERIC_OFFER_NAME = /reviewed[\s-]service/i
...
// Price/legal/guarantee claims are out of scope for this measurement layer.
const legalClaims = ["$", "guarantee", "refund", "warranty"]
...
console.log("test-public-conversion-signal: public conversion signal (Website Correction path)")
...
if (existsSync(join(ROOT, DOCS_PATH))) {
  ...
  ok(docs.includes(OFFER_NAME), "docs name the offer truthfully: The Website Correction")
  ok(!GENERIC_OFFER_NAME.test(docs), "docs never call the offer a generic reviewed service")
  ...
  ok(/subject line/.test(docs) && /operator/i.test(docs), "docs state the operator-visible signal is the received application email subject/body")
  ok(/edit|remove/.test(docs), "docs state the marker stays visible and editable by the applicant")
  for (const claim of legalClaims) {
    ok(!docs.includes(claim), `docs add no price/legal/guarantee claim (${claim})`)
  }
  ok(/0509/.test(docs) && /never/.test(docs), "docs keep product CTAs (0509) separate from Website Correction source names")
}
...
if (ctaTags.length === 0) {
  console.log("  note: no Website Correction CTA present in this checkout (homepage copy not merged); tag rule stays armed")
}
...
ok(
  /mailto:support@tinystudio\.in\?subject=The%20Website%20Correction%20application/i.test(contactHtml),
  "mailto route carries the truthful offer name in the default subject"
)
ok(contactHtml.includes(OFFER_NAME), "contact page names the offer truthfully: The Website Correction")
ok(!GENERIC_OFFER_NAME.test(contactHtml), "contact page never calls the offer a generic reviewed service")
...
ok(/remove/.test(contactHtml) && /visible|see/.test(contactHtml), "contact page states the marker is visible and removable before sending")
...
for (const claim of legalClaims) {
  ok(!contactHtml.includes(claim), `contact page adds no price/legal/guarantee claim (${claim})`)
}

console.log("E. privacy-choices narrow marker disclosure")
const privacyHtml = read(PRIVACY_CHOICES_HTML)
ok(/source marker/i.test(privacyHtml) && /subject\s*line/i.test(privacyHtml), "privacy-choices documents the marker mechanism narrowly")
ok(/nothing is stored|nothing is sent|no website code/i.test(privacyHtml), "privacy-choices states the marker stores and sends nothing")

console.log("F. no analytics provider, cookies, fingerprinting, or message-content collection")
