...
try {
	...
	const tamperedQueue = JSON.parse(preparedQueueBytes)
	tamperedQueue.generatedAt = "2026-07-29T00:00:01.000Z"
	wf(queuePaths(R).queueFile, `${JSON.stringify(tamperedQueue)}\n`)
	const alteredQueueCheck = checkQueue()
	eq(alteredQueueCheck.status, "failed")
	assert(alteredQueueCheck.failures.includes("queue output is stale or altered"))
	const alteredQueueCli = run(QUEUE, ["--mode", "check", "--as-of", AS_OF_DATE], E(QUEUE_TEST_NOW))
	neq(alteredQueueCli.status, 0)
	eq(JSON.parse(alteredQueueCli.stdout).status, "failed")
	wf(queuePaths(R).queueFile, preparedQueueBytes)
	un(queuePaths(R).queueFile)
	const missingQueueCheck = checkQueue()
	eq(missingQueueCheck.status, "advisory")
	assert(missingQueueCheck.advisories.some(message => message.includes("queue not prepared")))
