// Guard the LIVE Promptly support page against skipped heading levels.
//
// The 2026-08-08 scout item named /promptly/support/: it was repaired in
// source by PR #20 (card headings promoted from H3 to H2, outline
// H1 -> H2x3 -> footer H2 -> H3s with no jumps) and origin/main carries the
// fixed outline. This guard re-asserts the repaired outline against the
// page the live site actually serves, so a stale deployment (like the
// June-20 bundle still serving H3 cards) fails loudly instead of silently
// re-opening the finding.
//
// It runs standalone as `npm run site:check-live-promptly-support-heading`
// and from the nightly live-site-check.yml staleness net. It is
// deliberately NOT part of `npm test` / `npm run ci`: those blocking
// chains must stay green on repo state alone, while the live site is
// deployed by an external mechanism (Cloudflare Pages). Blocking CI on the
// live site would keep every pull request red whenever the deployment is
// stale, and would deadlock the deploy lane's pre-deploy `npm run check`
// gate. Network-tolerant: skips when the site is unreachable, fails loudly
// while it serves the stale bundle.
...
// The deployed page must keep the repaired heading outline (H1 -> H2 cards
...
// worktree HTML; this guard re-asserts it against the page the live site
...
// skipped-heading-level finding from the 2026-08-08 scout item.
const LIVE_PAGE = {
...
console.log("test-public-live-promptly-support-heading: the deployed tinystudio.in Promptly support page keeps the repaired heading outline (no skipped levels)")

console.log("A. live page carries the repaired heading hierarchy")
const html = await fetchLive(LIVE_PAGE.url)
if (html !== null) assertRepairedOutline(LIVE_PAGE.name, LIVE_PAGE.source, html)
...
console.log("C. standalone script wiring (out of the blocking ci/test chain)")
const pkg = JSON.parse(read("package.json"))
ok(
  pkg.scripts["site:check-live-promptly-support-heading"] === "node scripts/test-public-live-promptly-support-heading.mjs",
  "npm run site:check-live-promptly-support-heading runs this guard on demand"
)
ok(
  !pkg.scripts.ci.includes("test-public-live-promptly-support-heading.mjs"),
  "npm run ci does not block on the live-site guard (blocking chain stays green on repo state alone)"
)
ok(
  !pkg.scripts.test.includes("test-public-live-promptly-support-heading.mjs"),
  "npm test does not block on the live-site guard"
)
