...
| Selector | DNS Host |
|---|---|
| cf2024-1 | cf2024-1._domainkey.tinystudio.io |

Suggested dry-run command:

```bash
npm run send:configure -- --physical-address="..." --dkim-selector=cf2024-1 --dry-run
```
...
3. DKIM is configured for `cf2024-1` at `cf2024-1._domainkey.tinystudio.io`. Confirm the selector still matches the mail provider if you swap providers.
