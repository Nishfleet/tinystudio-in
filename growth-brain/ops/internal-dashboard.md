...
| P0 | Task: **Record first 5 Loom audits** - ByteMe Networks, IT Umbrella Group, Talos Cyber Solutions, Xentz Technologies, and YPM IT Solutions now have recording scripts; run `prospect:rehearsal -- --limit=5`, use `market:proof-cockpit`, then run `market:after-recording -- --from-clipboard` with the Loom URLs. | `npm run prospect:rehearsal -- --limit=5` | Open task |
...
|---|---|---|---|
| Active | P0 | **Record first 5 Loom audits** - ByteMe Networks, IT Umbrella Group, Talos Cyber Solutions, Xentz Technologies, and YPM IT Solutions now have recording scripts; run `prospect:rehearsal -- --limit=5`, use `market:proof-cockpit`, then run `market:after-recording -- --from-clipboard` with the Loom URLs. | `npm run growth:start -- --view=record` |
