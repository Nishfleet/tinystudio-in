# Weekly Growth Brain Report

## Week

- Client:
- Dates:
- Main goal:

## What Changed

- Shipped:
- Drafted:
- Waiting on client:
- Blocked:

## Revenue Leak Loop

| Lane | Before Leak | Fix Shipped / Handed Off | Client-Visible Value | Next Action |
|---|---|---|---|---|
| Conversion |  |  |  |  |
| Search trust |  |  |  |  |

## Search Trust Review

| Area | Current State | Fix / Action | Next Measurement |
|---|---|---|---|
| On-site search trust |  |  |  |
| Off-site trust/distribution |  |  |  |

## Numbers To Review

| Metric | Last Period | This Period | Notes |
|---|---:|---:|---|
| Sessions |  |  |  |
| Conversion rate |  |  |  |
| Revenue |  |  |  |
| AOV |  |  |  |
| Email revenue |  |  |  |
| Ad spend |  |  |  |
| CPA/CAC |  |  |  |

## Measurement Contract

| Signal | Source | Owner | Next Check | Baseline / Current | Decision Rule |
|---|---|---|---|---|---|
|  |  |  |  |  |  |

## Learnings

- What improved:
- What got worse:
- What stayed flat:
- What surprised us:

## Client Pulse

- What felt valuable:
- What felt unclear:
- Delight add-on:
- Health score:
- Retention risk:

## Retention Value Stack

- Most valuable change this week:
- One quick win:
- What we need from client:
- Why retain next month:

## Client Confirmation

- Client saw delta:
- Client understood value:
- Client approved next action:
- Continue / retain signal:

## Next Tests

| Priority | Test | Why | Owner | Due |
|---|---|---|---|---|
| 1 |  |  |  |  |
| 2 |  |  |  |  |
| 3 |  |  |  |  |

## Update The Client Brain

Copy durable learnings into `client-brain-template/weekly-learnings.md`.
