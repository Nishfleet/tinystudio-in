...
First checked: 2026-08-09 (baseline below). Re-verified: 2026-08-11, 2026-08-12, 2026-08-14, 2026-08-20 (see re-verification log).
...
### 2026-08-20 — re-checked, claim still holds

**Verdict: True as checked on 2026-08-20.** No Tiny Studio listing, page, or indexed mention of Promptly or Drishti on Product Hunt or AlternativeTo. The 2026-08-14 re-check was re-run live; every name-matching result still belongs to an unrelated third party.

Re-checked live on 2026-08-20:

- AlternativeTo on-platform search for `promptly` (`https://alternativeto.net/browse/search/?q=promptly`, direct fetch, HTTP 200) — 17 results, all unrelated: Promptly AI (no-code generative-AI chatbot builder), Promptly (open-source blog publishing tool), VK, Meeting On Time, PDFtoExcel.com, Interview Answers Generator, Prmptly.xyz, PromptYourJob, Promptzy, Promptless, PromptLock, PromptlessPress, PromptZyo, PromptLabHub, Windows Command Prompt, and similar. None is Tiny Studio's booking/no-show app; none links to tinystudio.in (zero occurrences of `tinystudio` or `tinystudio.in` anywhere in the rendered results).
- AlternativeTo on-platform search for `drishti` (direct fetch, HTTP 200) — "Drishti" (open-source scientific visualisation software for volumetric/tomography data) plus fuzzy matches Now&Me and Pollscape; zero occurrences of `tinystudio` or `tinystudio.in`. None is Tiny Studio's mindful screen-time app.
- AlternativeTo on-platform search for `tinystudio` (direct fetch, HTTP 200) — one fuzzy match (JPEG Lossless Rotator); no Tiny Studio listing; zero `tinystudio` tokens beyond the page URL and search-result heading.
- AlternativeTo software page `https://alternativeto.net/software/tinystudio/about/` (direct fetch) — HTTP 404 with the `404 - Page not found` title and `meta name="robots" content="noindex"`; no listing exists.
- Wayback availability API for `alternativeto.net/software/tinystudio/about/` — `{"archived_snapshots": {}}`, confirming no archived capture of a Tiny Studio listing page.
- Product Hunt exact-name page `producthunt.com/products/tinystudio` — direct fetch is Cloudflare CAPTCHA-gated (504/timeout on the search page; same hardening as previous runs). The Wayback capture dated 2025-01-25 (`web.archive.org/web/20250125073207/https://www.producthunt.com/products/tinystudio`, direct fetch, HTTP 200) confirms the page is still the unrelated Mac subtitle app: title `TinyStudio - Product Information, Latest Updates, and Reviews 2025 | Product Hunt`, maker handle `Tinyfool`, tagline "Using AI to generate subtitles on Mac", site links to the Apple App Store (`apps.apple.com/us/app/tinystudio/id6448954288`); zero occurrences of `tinystudio.in` in the page. Not a Tiny Studio asset.
- Index checks (DuckDuckGo `html.duckduckgo.com/html/`, direct fetches): `site:producthunt.com "tinystudio.in"` — "No results found"; `site:alternativeto.net "tinystudio.in"` — "No results found"; `site:alternativeto.net "promptly" booking` — "No results found". The remaining doc query `site:alternativeto.net tinystudio OR "tiny studio" promptly OR drishti` hit a DuckDuckGo bot challenge (anomaly page) on first attempt and the r.jina.ai proxy timed out; it is redundant with the three clean queries above plus the on-platform searches, so the verdict does not depend on it.
- Brand-level check: `site:producthunt.com tinystudio` — the only Product Hunt pages are subpages of the unrelated Tinyfool app (`producthunt.com/p/tinystudio` forum, `producthunt.com/products/tinystudio/makers`) plus irrelevant fuzzy leaderboard matches (CapCut, SureThing.io, itch.io, Tiny World Builder, etc.); zero occurrences of `tinystudio.in` in any result snippet.
- Live app pages: `https://tinystudio.in/promptly/` — HTTP 200, zero occurrences of "Product Hunt" or "AlternativeTo" in page HTML, zero `noindex`/`nofollow` robots meta; `https://tinystudio.in/drishti/` — HTTP 200, zero occurrences of "Product Hunt" or "AlternativeTo" in page HTML, zero `noindex`/`nofollow` robots meta; `https://tinystudio.in/robots.txt` — `User-agent: *` / `Allow: /` / `Sitemap: https://tinystudio.in/sitemap.xml` (crawlable, sitemap exposed).

Method note: on 2026-08-20 the AlternativeTo on-platform searches, the AlternativeTo 404 software page, the Wayback availability API, and the DuckDuckGo site-restricted index queries were all reachable via direct fetch (no proxy needed), which is less gated than previous runs. Product Hunt search pages remain Cloudflare CAPTCHA-gated to direct fetches, so the PH side of the verdict rests, as before, on (a) the empty `site:producthunt.com "tinystudio.in"` DuckDuckGo index query, (b) the brand-level `site:producthunt.com tinystudio` query (all results belong to the unrelated Tinyfool app, zero `tinystudio.in` mentions), and (c) the Wayback capture of the exact-name page (2025-01-25, still the Mac subtitle app, zero `tinystudio.in` mentions). All independent evidence lines agree.
