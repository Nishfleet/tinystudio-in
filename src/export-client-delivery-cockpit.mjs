...
handleHelp(args, `Usage: node src/export-client-delivery-cockpit.mjs clients/client-slug [--output=clients/client-slug/delivery-cockpit.html]`);
...
const outputPath = outputArg
  ? resolveOutputPath(outputArg.split("=").slice(1).join("="), { fallback: join(resolvedClientPath, "delivery-cockpit.html") })
  : join(resolvedClientPath, "delivery-cockpit.html");
...
const trackingEvidence = serviceClient.trackingEvidence[0]
  ...
  : null;
const readiness = runJson(["src/check-client-readiness.mjs", resolvedClientPath]);
