...
if (view === "cockpit") {
  runJson(["src/export-growth-cockpit.mjs"]);
  const missionMarkdown = readFileSync(missionMarkdownPath, "utf8");
  mission = missionMarkdown.match(/## Today's Constraint\n+([\s\S]*?)(?:\n## |$)/)?.[1]?.trim() || "";
} else {
  const missionResult = runJson(["src/export-daily-money-mission.mjs", "--limit=5"]);
...
if (view === "record" || view === "all") {
  runJson(["src/export-recording-rehearsal-check.mjs", "--limit=5"]);
