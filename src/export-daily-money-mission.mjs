...
const usage = `Usage: node src/export-daily-money-mission.mjs [--limit=5] [--output=runs/daily-money-mission.md] [--html=runs/daily-money-mission.html]`;
handleHelp(process.argv.slice(2), usage);
refuseUnknownArgs(process.argv.slice(2), ["--limit", "--output", "--html"], usage);
...
runJson(["src/export-recording-cockpit.mjs", `--limit=${limit}`]);
runJson(["src/export-lead-scoring-cockpit.mjs", "--limit=10"]);
runJson(["src/export-recording-teleprompter.mjs", `--limit=${limit}`]);
const rehearsal = runJson(["src/export-recording-rehearsal-check.mjs", `--limit=${limit}`]);
runJson(["src/export-prospect-outbox.mjs"]);
runJson(["src/export-followup-cockpit.mjs"]);
runJson(["src/export-sales-cockpit.mjs"]);
try {
  runJson(["src/refresh-operator-artifacts.mjs"]);
} catch {
  // lockstep refresh is best-effort from the daily mission; the mission still reads private metrics
}
const metrics = runJson(["src/export-growth-metrics.mjs", "--output=runs/metrics-for-mission.md"]);
runJson(["src/export-managed-it-one-pager.mjs"]);
const todayResult = runJson(["src/show-growth-command-center.mjs", `--limit=${Math.max(limit + 2, 7)}`]);
