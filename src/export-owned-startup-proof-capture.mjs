...
handleHelp(process.argv.slice(2), `Usage: node src/export-owned-startup-proof-capture.mjs`);
...
function ensureOwnedClients() {
  ...
  if (!missing) return;
  execFileSync("node", ["src/seed-owned-startup-proof-lane.mjs"], {
    encoding: "utf8",
...
for (const startup of startups) {
  ...
  execFileSync("node", ["src/export-client-weekly-report.mjs", startup.clientPath, `--output=${join(startup.clientPath, "reports/week-1-report.md")}`], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "pipe"]
  });
