...
const activeGeneratorSources = [
	"src/create-client-sprint.mjs",
	"src/check-client-readiness.mjs",
	"src/create-prospect-audit.mjs",
	"src/draft-loom-package.mjs",
	"src/draft-prospect-message.mjs",
	"src/draft-loom-recording-script.mjs",
	"src/draft-sales-call-prep.mjs",
	"src/draft-client-kickoff.mjs",
	"src/prepare-prospect-close-package.mjs",
	"src/convert-prospect-to-client.mjs",
	"src/export-daily-money-mission.mjs",
	"src/export-market-proof-run.mjs",
	"src/prepare-prospect-batch-send.mjs",
	"src/export-proof-library.mjs",
	"src/export-client-delivery-cockpit.mjs",
	"src/export-market-benchmark.mjs",
	"src/export-internal-dashboard.mjs",
	"src/review-client-acceptance.mjs",
	"src/review-client-proof.mjs",
	"src/show-growth-command-center.mjs",
	"src/export-managed-it-one-pager.mjs"
]
...
const activeWorkflowRequirements = [
	["canonical product", /human-reviewed[\s\S]{0,120}The Website Correction/i],
...
for (const path of [
	"src/draft-sales-call-prep.mjs",
	"src/draft-recording-sharpness-brief.mjs",
	"src/draft-loom-recording-script.mjs",
	"src/draft-client-kickoff.mjs",
	"src/draft-prospect-message.mjs",
	"src/lib/canonical-service-copy.mjs",
	"src/lib/client-scaffold.mjs"
]) {
...
for (const path of ["src/draft-prospect-message.mjs", "src/export-daily-money-mission.mjs", "src/export-market-proof-run.mjs", "src/prepare-prospect-batch-send.mjs"]) {
	const content = existsSync(path) ? readFileSync(path, "utf8") : ""
...
const kickoffSource = readFileSync("src/draft-client-kickoff.mjs", "utf8")
if (/section\(buyerRoom|read\("buyer-room\.md"\)/.test(kickoffSource)) {
	failures.push("src/draft-client-kickoff.mjs: client-facing kickoff still trusts mutable buyer-room offer text")
}

const prospectMessageSource = readFileSync("src/draft-prospect-message.mjs", "utf8")
if (/section\(outreach,\s*"First Message"|lineValue\(buyerRoom,\s*\/\^- Price/.test(prospectMessageSource)) {
	failures.push("src/draft-prospect-message.mjs: active message still trusts legacy outreach or buyer-room price")
}

const loomPackageSource = readFileSync("src/draft-loom-package.mjs", "utf8")
if (/readFileSync\(join\(prospectPath,\s*"(?:outreach|buyer-room|audit-brief|loom-outline)\.md"/.test(loomPackageSource)) {
	failures.push("src/draft-loom-package.mjs: outbound package still copies mutable legacy offer sources")
}

const recordingScriptSource = readFileSync("src/draft-loom-recording-script.mjs", "utf8")
if (/read\("buyer-room\.md"\)|priceMatch/.test(recordingScriptSource) || !/FOUNDER_PILOT\.offerPriceUsd/.test(recordingScriptSource)) {
	failures.push("src/draft-loom-recording-script.mjs: recording pitch does not use the immutable founder-pilot price")
}

const metricsSource = readFileSync("src/export-growth-metrics.mjs", "utf8")
if (!/loadValidatedServiceClients/.test(metricsSource) || /const clientRows = listFolders\("clients"\)/.test(metricsSource)) {
	failures.push("src/export-growth-metrics.mjs: client metrics are not sourced from canonical service validation")
}
