...
handleHelp(process.argv.slice(2), `Usage: node src/export-market-benchmark.mjs [--output=docs/strategy/market-parity-benchmark-2026.md] [--ops=growth-brain/ops/competitive-proof-matrix.md] [--html=growth-brain/ops/competitive-proof-matrix.html]`);
...
const metrics = runJson(["src/export-growth-metrics.mjs", "--output=runs/metrics-for-benchmark.md"]);
const sender = runJson(["src/check-outbound-sender-setup.mjs"]);
const proofRun = existsSync("prospects/loom-links.txt")
  ? runJson(["src/check-market-proof-run.mjs"])
