...
handleHelp(args, `Usage: node src/export-client-channel-readiness.mjs clients/client-slug`);
const clientPath = args.find((arg) => !arg.startsWith("--"));

if (!clientPath) {
  console.error("Usage: node src/export-client-channel-readiness.mjs clients/client-slug");
  process.exit(1);
}
