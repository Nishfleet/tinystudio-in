...
handleHelp(process.argv.slice(2), `Usage: node src/check-market-parity-readiness.mjs [--strict] [--skip-kit] [--output=growth-brain/ops/market-parity-readiness.md]`);
...
const metrics = runJson(["src/export-growth-metrics.mjs", "--output=runs/metrics-for-parity.md"]);
const sender = runJson(["src/check-outbound-sender-setup.mjs"]);
const kit = skipKit
  ? { status: "skipped", checkedFiles: 0, allowedCommands: 0 }
  : runGate(["src/check-human-service-kit.mjs"], true);
const send = runJson(["src/check-outbound-send-readiness.mjs"]);
const claims = runJson(["src/check-outbound-claim-safety.mjs"]);
const benchmark = {
  ...runJson(["src/export-market-benchmark.mjs", "--output=runs/parity-benchmark.md", "--ops=runs/parity-matrix.md", "--html=runs/parity-matrix.html"]),
...
const clientProof = serviceClients.map((client) => ({
  ...
  readiness: client.ok && existsSync(join(client.clientPath, "intake.md"))
    ? runJson(["src/check-client-readiness.mjs", client.clientPath]).status
    : "blocked"
}));
