...
const loomResult = runJson(["src/add-prospect-loom-link.mjs", prospectPath, loomUrl]);
const messageResult = runJson(["src/draft-prospect-message.mjs", prospectPath]);
...
const outboxResult = runJson(["src/export-prospect-outbox.mjs", `--output=${outboxPath}`]);
