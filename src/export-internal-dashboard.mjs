...
handleHelp(process.argv.slice(2), `Usage: node src/export-internal-dashboard.mjs [--output=runs/internal-dashboard.md] [--html=runs/internal-dashboard.html] [--plain]`);
...
const doctor = runJson(["src/export-growth-doctor.mjs"]);
const metrics = runJson(["src/export-growth-metrics.mjs", "--output=runs/metrics-for-dashboard.md"]);
let serviceQueue;
try { serviceQueue = runJson(["src/run-review-queue.mjs", "--dry-run", "--scope", "all"]); }
catch (error) {
...
const parity = runJson(["src/check-market-parity-readiness.mjs", "--skip-kit", "--output=runs/internal-dashboard-parity.md"]);
const todayView = runJson(["src/show-growth-command-center.mjs", "--limit=12"]);
const marketProof = existsSync(join(serviceRoot, "prospects/loom-links.txt"))
  ? runJson(["src/export-market-proof-cockpit.mjs"])
  : {
...
const marketLearning = runJson(["src/export-market-learning-review.mjs"]);
const rehearsal = runJson(["src/export-recording-rehearsal-check.mjs"]);
const senderGuide = runJson(["src/export-sender-setup-guide.mjs"]);
