...
handleHelp(process.argv.slice(2), `Usage: node src/export-growth-metrics.mjs [--output=growth-brain/ops/live-metrics.md] [--plain]`);
...
const clientRows = validatedClientRecords.filter((client) => client.ok && client.day0).map((client) => {
  ...
  const readiness = runRepoJson(["src/check-client-readiness.mjs", path]);
  return {
    path,
