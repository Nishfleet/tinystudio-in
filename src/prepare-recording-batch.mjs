...
if (!skipSnapshot) {
  for (const prospect of prospects) {
    const snapshotArgs = [
      "src/snapshot-prospect-page.mjs",
      prospect.path,
      `--timeout=${timeoutMs}`
    ];
...
if (!skipContactPlan) {
  for (const prospect of prospects) {
    const contactArgs = [
      "src/enrich-prospect-contact-plan.mjs",
      prospect.path,
      `--timeout=${timeoutMs}`
    ];
...
const queue = runJson([
  "src/export-recording-queue.mjs",
  `--limit=${limit}`,
  `--output=${paths.queue}`
]);
const cockpit = runJson([
  "src/export-recording-cockpit.mjs",
  `--limit=${limit}`,
  `--output=${paths.cockpit}`
]);
const teleprompter = runJson([
  "src/export-recording-teleprompter.mjs",
  `--limit=${limit}`,
  `--output=${paths.teleprompter}`
]);
const rehearsal = runJson([
  "src/export-recording-rehearsal-check.mjs",
  `--limit=${limit}`,
