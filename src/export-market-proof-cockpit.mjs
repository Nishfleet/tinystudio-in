...
handleHelp(args, `Usage: node src/export-market-proof-cockpit.mjs [prospects/loom-links.txt] [--output=runs/market-proof-cockpit.md] [--html=runs/market-proof-cockpit.html] [--limit=5] [--plain]`);
...
const check = runJson(["src/check-market-proof-run.mjs", inputPath, `--output=${checkOutputPath}`, `--limit=${limit}`]);
