...
handleHelp(process.argv.slice(2), `Usage: node src/export-market-proof-run.mjs [--output=growth-brain/ops/11-10-proof-run.md] [--loom-links=prospects/loom-links.txt] [--limit=5] [--skip-kit]`);
...
const parityArgs = ["src/check-market-parity-readiness.mjs", `--output=${parityOutputPath}`];
...
const metrics = runJson(["src/export-growth-metrics.mjs", "--output=runs/metrics-for-proof-run.md"]);
