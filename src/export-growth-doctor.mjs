...
handleHelp(process.argv.slice(2), `Usage: node src/export-growth-doctor.mjs [--output=runs/growth-doctor.md] [--no-checks] [--plain]`);
const outputArg = process.argv.find((arg) => arg.startsWith("--output="));
const outputPath = resolveOutputPath(outputArg?.split("=").slice(1).join("="), { fallback: "runs/growth-doctor.md" });
...
const metrics = runJson(["src/export-growth-metrics.mjs", "--output=runs/metrics-for-doctor.md"]);
const todayResult = runJson(["src/show-growth-command-center.mjs", "--limit=8"]);
const recordingPrep = recordingPrepStatus(5);
const rehearsal = runJson(["src/export-recording-rehearsal-check.mjs", "--limit=5"]);
const next = bottleneck(metrics.counts, recordingPrep, rehearsal);
const checks = skipChecks ? [] : [
  runCheck("Agency defaults", ["src/check-agency-defaults.mjs"]),
  runCheck("Sender setup", ["src/check-outbound-sender-setup.mjs"]),
  runCheck("Claim safety", ["src/check-outbound-claim-safety.mjs"]),
  runCheck("Send readiness", ["src/check-outbound-send-readiness.mjs"])
];
