...
const requiredFiles = [
	...
	"src/check-product-truth.mjs",
	"src/check-retention-automation.mjs",
	"src/create-client-sprint.mjs",
	"src/convert-prospect-to-client.mjs",
	"src/import-sprint-application.mjs",
	"src/record-service-decision.mjs",
	"src/record-service-day0.mjs",
	"src/record-service-evidence.mjs",
	"src/record-service-resume.mjs",
	"src/repair-service-transition.mjs",
	"src/service-state-backup.mjs",
	"src/run-review-queue.mjs",
...
for (const command of Object.values(packageJson.scripts)) {
	for (const match of String(command).matchAll(/\bnode\s+((?:src|test)\/[a-z0-9._/-]+\.mjs)\b/gi)) activeScriptFiles.add(match[1])
}
...
for (const path of filesUnder(join(codeRoot, "src"))) {
...
for (const path of ["src/create-client-sprint.mjs", "src/convert-prospect-to-client.mjs"]) {
	const codePath = join(codeRoot, path)
