...
//     page from #34).
//
// CLI: node src/prepare-public-deploy-bundle.mjs --source public --output <dir>
...
const buildOps = () => [
  ...
  ...FILTERED_PAGES.filter((rel) => rel !== "index.html" && rel !== "contact/index.html" && rel !== "support/index.html" && rel !== "privacy/index.html").map((rel) => ({
    file: rel,
    kind: "regex",
  ...
  if (errors.length > 0) {
    throw new Error(
      [
        ...
        "snooze was lifted. Update the markers in src/prepare-public-deploy-bundle.mjs",
