# Lane 1 — item c33e2af2b6 fourth-incarnation reverify: test-active-operator-surfaces.mjs "Tracked generated artifact" failure
...
Fourth dispatch/resume of packet tinystudio-in-lane1-1787289934363 (resumed
worker 2026-08-21 ~12:45Z). Three prior incarnations (retired 06:29Z,
reverified ~11:50Z, re-reverified ~12:15Z) all reached the same verdict; this
incarnation re-confirmed from scratch rather than trusted.

## Independent verification (2026-08-21 ~12:45Z)
...
- `node scripts/test-active-operator-surfaces.mjs` → **"Active operator
  ...
  restores canonical baselines; no tracked-artifact drift introduced).
...
- Controller-visible state checked directly:
  `/home/nish/workspaces/products/tinystudio-in/.fleet/improvement-loop.json`
  already carried item c33e2af2b6 status **resolved** with the same receipt
  before this run — the re-dispatch is a resume of the same in-flight packet,
  not a rotation re-pick of a resolved item.

## Outcome

- Claims published to lane-1.json before editing:
  `.fleet/improvement-loop.json`, `.lane/reports/lane1-test-active-operator-surfaces-tracked-artifact-fourth-reverify-20260821.md`.
